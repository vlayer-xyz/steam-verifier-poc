...
export default function SteamPage() {
  ...
  const getNormalizedCallbackUrl = useCallback(() => {
    if (!callbackUrl) {
      return null
    }
    ...
    try {
      ...
      console.error('Invalid callbackUrl when routing error:', error)
      return null
    }
  }, [callbackUrl])
  ...
  const handleError = useCallback(
    (message: string, kind: 'error' | 'info' = 'error') => {
      const normalizedCallback = getNormalizedCallbackUrl()

      if (!normalizedCallback) {
        setVerificationResult(kind === 'error' ? `❌ ${message}` : `ℹ️ ${message}`)
        return
      }

      const callbackTarget = new URL(normalizedCallback)
      callbackTarget.searchParams.set('success', 'false')
      callbackTarget.searchParams.set('message', message)
      callbackTarget.searchParams.set('type', kind)
      ...
      router.push(`/verified?${params.toString()}`)
    },
    [getNormalizedCallbackUrl, router]
  )
  ...
  const verifyGamingActivity = async () => {
    ...
    if (!webhookUrl || webhookError) {
      const message = webhookError ?? 'Provide a valid webhookUrl query parameter before verification.'
      handleError(message)
      return
    }
    if (!callbackUrl || callbackError) {
      const message = callbackError ?? 'Provide a valid callbackUrl query parameter before verification.'
      handleError(message)
      return
    }
    ...
    try {
      ...
      if (response.ok) {
        if (data.success) {
          ...
          handleError(data.message, 'info')
        }
      } else {
        handleError(`Verification failed: ${data.error}`)
      }
    } catch {
      handleError('Network error during verification')
    } finally {
      setVerifying(false)
    }
  }
